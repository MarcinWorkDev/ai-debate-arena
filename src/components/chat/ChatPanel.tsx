...
export function ChatPanel() {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900">
      {/* Header */}
      <div className="px-3 py-2 md:px-5 md:py-4 border-b border-slate-700/50 bg-slate-800/50 shrink-0">
        <div className="flex items-center justify-between">
          <h2 className="text-slate-100 font-semibold text-sm md:text-base">Debate Log</h2>
          <div className="flex items-center gap-1.5 md:gap-3">
            {/* Hand raise button */}
            {status === 'running' && !isUserTurn && (
              <button
                onClick={toggleHandRaised}
                className={`flex items-center gap-1 md:gap-1.5 px-2 py-1 md:px-3 md:py-1.5 rounded-lg text-xs font-medium transition-all ${
                  handRaised
            ...
            <span className="text-xs text-slate-400 bg-slate-700/50 px-1.5 py-0.5 md:px-2 md:py-1 rounded">
              {messages.length} <span className="hidden sm:inline">messages</span><span className="sm:hidden">msg</span>
            </span>
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-2 md:p-4 space-y-3 md:space-y-4 scrollbar-thin
      ...
      {isUserTurn && (
        <form onSubmit={handleSubmit} className="p-2 md:p-4 border-t border-slate-700/50 bg-slate-800/50">
